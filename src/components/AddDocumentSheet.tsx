        ...
        <View style={styles.container}>
            ...
            <View style={styles.documentTypesGrid}>
                {documentTypes.map((type) => (
                    <TouchableOpacity
                        key={type.id}
                        style={[styles.documentTypeItem, { backgroundColor: isDark ? colors.card : '#F5F5F5' }]}
                        onPress={() => handleDocumentTypeSelect(type.id)}
                        activeOpacity={0.7}
                    >
                        <View style={[styles.documentTypeIcon, { backgroundColor: isDark ? '#2A2A2A' : '#FFF' }]}>
                            <type.icon size={24} color={colors.text} />
                        </View>
                        <Text style={[styles.documentTypeLabel, { color: colors.text }]}>{type.label}</Text>
                    </TouchableOpacity>
                ))}
            </View>
            ...
            >
                ...
                <View style={styles.primaryUploadText}>
                    ...
                    <Text style={styles.primaryUploadSubtitle}>Best for physical documents</Text>
                </View>
            ...
            <View style={styles.secondaryUploadRow}>
                <TouchableOpacity
                    style={[styles.secondaryUploadButton, { backgroundColor: isDark ? colors.card : '#F5F5F5' }]}
                    onPress={() => handleUploadMethodSelect('photo')}
                    activeOpacity={0.7}
                >
                    <View style={[styles.secondaryUploadIcon, { backgroundColor: isDark ? '#2A2A2A' : '#FFF' }]}>
                        <Image size={24} color={colors.text} />
                    </View>
                    <Text style={[styles.secondaryUploadLabel, { color: colors.text }]}>Upload Photo</Text>
                </TouchableOpacity>
                ...
                >
                    ...
                    <Text style={[styles.secondaryUploadLabel, { color: colors.text }]}>Upload PDF</Text>
                </TouchableOpacity>
            </View>
        </View>
    );
